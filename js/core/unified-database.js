...
class EnhancedMorphemeDatabase {
  ...
  initializeDatabase() {
    ...
    this.affixes = [
      ...
      // Person affixes
      {id: "aff18", text: "im", type: "person", compatibleWith: ["verb"], position: "after", variants: ["ım", "im", "um", "üm"], function: "1. tekil şahıs eki", difficulty: "easy"},
      {id: "aff19", text: "sin", type: "person", compatibleWith: ["verb"], position: "after", variants: ["sın", "sin", "sun", "sün"], function: "2. tekil şahıs eki", difficulty: "easy"},
      {id: "aff20", text: "", type: "person", compatibleWith: ["verb"], position: "after", variants: [""], function: "3. tekil şahıs eki", difficulty: "easy"},
      {id: "aff21", text: "iz", type: "person", compatibleWith: ["verb"], position: "after", variants: ["ız", "iz", "uz", "üz"], function: "1. çoğul şahıs eki", difficulty: "medium"},
      {id: "aff22", text: "siniz", type: "person", compatibleWith: ["verb"], position: "after", variants: ["sınız", "siniz", "sunuz", "sünüz"], function: "2. çoğul şahıs eki", difficulty: "medium"},
      {id: "aff23", text: "ler", type: "person", compatibleWith: ["verb"], position: "after", variants: ["lar", "ler"], function: "3. çoğul şahıs eki", difficulty: "medium"},
      
      // Derivational affixes
      {id: "aff24", text: "lık", type: "derivational", compatibleWith: ["noun", "adjective"], position: "after", variants: ["lık", "lik", "luk", "lük"], function: "isimden isim yapım eki", difficulty: "medium"},
      {id: "aff25", text: "ci", type: "derivational", compatibleWith: ["noun"], position: "after", variants: ["cı", "ci", "cu", "cü"], function: "meslek yapım eki", difficulty: "easy"},
      {id: "aff26", text: "li", type: "derivational", compatibleWith: ["noun"], position: "after", variants: ["lı", "li", "lu", "lü"], function: "sahiplik eki", difficulty: "easy"},
      {id: "aff27", text: "siz", type: "derivational", compatibleWith: ["noun"], position: "after", variants: ["sız", "siz", "suz", "süz"], function: "yokluk eki", difficulty: "easy"},
      {id: "aff28", text: "ce", type: "derivational", compatibleWith: ["adjective", "adverb"], position: "after", variants: ["ca", "ce"], function: "benzerlik eki", difficulty: "medium"},
      
      // Verb to noun derivational affixes
      {id: "aff29", text: "mek", type: "infinitive", compatibleWith: ["verb"], position: "after", variants: ["mak", "mek"], function: "mastar eki", difficulty: "easy"},
      {id: "aff30", text: "me", type: "gerund", compatibleWith: ["verb"], position: "after", variants: ["ma", "me"], function: "isim-fiil eki", difficulty: "medium"},
      {id: "aff31", text: "iş", type: "gerund", compatibleWith: ["verb"], position: "after", variants: ["ış", "iş", "uş", "üş"], function: "isim-fiil eki", difficulty: "medium"},
      {id: "aff32", text: "en", type: "participle", compatibleWith: ["verb"], position: "after", variants: ["an", "en"], function: "sıfat-fiil eki", difficulty: "medium"},
      {id: "aff33", text: "dik", type: "participle", compatibleWith: ["verb"], position: "after", variants: ["dık", "dik", "duk", "dük", "tık", "tik", "tuk", "tük"], function: "sıfat-fiil eki", difficulty: "hard"},
      
      // Adverb affixes
      {id: "aff34", text: "ce", type: "adverbial", compatibleWith: ["adjective"], position: "after", variants: ["ca", "ce"], function: "zarf yapım eki", difficulty: "medium"},
      {id: "aff35", text: "leyin", type: "adverbial", compatibleWith: ["noun"], position: "after", variants: ["leyin"], function: "zaman zarfı eki", difficulty: "hard"},
      ...
      // Ability
      {id: "aff38", text: "ebil", type: "ability", compatibleWith: ["verb"], position: "after", variants: ["abil", "ebil"], function: "yeterlilik eki", difficulty: "medium"},
      
      // Passive
      {id: "aff39", text: "il", type: "passive", compatibleWith: ["verb"], position: "after", variants: ["ıl", "il", "ul", "ül"], function: "edilgen çatı eki", difficulty: "hard"},
      {id: "aff40", text: "in", type: "passive", compatibleWith: ["verb"], position: "after", variants: ["ın", "in", "un", "ün"], function: "edilgen çatı eki", difficulty: "hard"}
    ];
  }

  // Get morphemes based on age group and difficulty level
  getMorphemes(ageGroup, difficultyLevel) {
    let verbFilters = this.getFilters(ageGroup, difficultyLevel);
    let adjectiveFilters = this.getFilters(ageGroup, difficultyLevel);
    let adverbFilters = this.getFilters(ageGroup, difficultyLevel);
    let affixFilters = this.getFilters(ageGroup, difficultyLevel);
    
    return {
      verbs: this.verbRoots.filter(verb => 
        verbFilters.difficulties.includes(verb.difficulty) && 
        verbFilters.frequencies.includes(verb.frequency)
      ),
      adjectives: this.adjectiveRoots.filter(adj => 
        adjectiveFilters.difficulties.includes(adj.difficulty) && 
        adjectiveFilters.frequencies.includes(adj.frequency)
      ),
      adverbs: this.adverbRoots.filter(adv => 
        adverbFilters.difficulties.includes(adv.difficulty) && 
        adverbFilters.frequencies.includes(adv.frequency)
      ),
      affixes: this.affixes.filter(aff => 
        affixFilters.difficulties.includes(aff.difficulty)
      )
    };
  }
  
  // Helper method to get filters based on age group and difficulty level
  getFilters(ageGroup, difficultyLevel) {
    let difficulties = [];
    let frequencies = ["high"];
    
    // Set difficulty filters based on age group and difficulty level
    if (ageGroup === "child") { // 7-12
      if (difficultyLevel === "easy") {
        difficulties = ["easy"];
      } else if (difficultyLevel === "medium") {
        difficulties = ["easy", "medium"];
        frequencies.push("medium");
      } else { // hard
        difficulties = ["easy", "medium"];
        frequencies.push("medium");
      }
  ...
  // Get random morphemes for game
  getRandomMorphemes(ageGroup, difficultyLevel, count) {
    const morphemes = this.getMorphemes(ageGroup, difficultyLevel);
    const result = {
      verbs: this.getRandomItems(morphemes.verbs, Math.floor(count * 0.3)),
      adjectives: this.getRandomItems(morphemes.adjectives, Math.floor(count * 0.2)),
      adverbs: this.getRandomItems(morphemes.adverbs, Math.floor(count * 0.2)),
      affixes: this.getRandomItems(morphemes.affixes, Math.floor(count * 0.3))
    };
  ...
  isValidWord(morphemes) {
    // Basic validation logic
    if (morphemes.length === 0) return false;
    
    // Check if the first morpheme is a root
    const firstMorpheme = morphemes[0];
    if (!['verb', 'noun', 'adjective', 'adverb'].includes(firstMorpheme.type)) {
      return {
        valid: false,
        message: "Sözcük bir kök ile başlamalıdır."
      };
    }
    
    // Check compatibility between morphemes
    for (let i = 1; i < morphemes.length; i++) {
      const currentMorpheme = morphemes[i];
      const previousMorpheme = morphemes[i-1];
      
      // Check if the affix is compatible with the previous morpheme
      if (!currentMorpheme.compatibleWith.includes(previousMorpheme.type)) {
        return {
          valid: false,
          message: `"${currentMorpheme.text}" eki "${previousMorpheme.text}" ${previousMorpheme.type === 'verb' ? 'fiili' : 'ismi'} ile kullanılamaz.`
        };
      }
    }
  ...
  // Calculate word score based on morpheme difficulty and count
  calculateWordScore(morphemes) {
    let score = 0;
    
    for (const morpheme of morphemes) {
      // Base points for each morpheme
      score += 10;
      
      // Bonus points based on difficulty
      if (morpheme.difficulty === "medium") {
        score += 5;
      } else if (morpheme.difficulty === "hard") {
        score += 10;
      }
      
      // Bonus points based on frequency (rarer = more points)
      if (morpheme.frequency === "medium") {
        score += 3;
      } else if (morpheme.frequency === "low") {
        score += 7;
      }
    }
